...
- Use **PascalCase**
- Examples: `HashTable`, `FlatVector`, `HashAggregateOp`, `DataType`
...
- Examples: `RETURN_IF_ERROR`, `VAGG_LIKELY`
...
public:
    explicit HashAggregateOp(std::unique_ptr<Operator> child);
...
```cpp
RETURN_IF_ERROR(child_->Prepare(ctx));
...
TEST(HashAggregateOpTest, SumSingleGroup) { }
```
