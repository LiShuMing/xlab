...
class SPSC_Q {
  public:
    ...
    Result Write(MessageSize size, WriteCallback c) {
        // This ensures that only one writer can write by having an atomic blockIndex and using
        // fetch_add
        ...
        if (!currBlock.unread.compare_exchange_strong(expectedUnread, false,
            ...
            // or a read happened and the version became even because the reader does odd_version +
            // 1 = even_version in the case of the version being even after we didn't succeed we can
            // still proceed with the writing

            // HOWEVER, if we failed to set unread to false because the reader thread set `unread`
            // to false first AND the read is still happening then version is still odd so we CANNOT
