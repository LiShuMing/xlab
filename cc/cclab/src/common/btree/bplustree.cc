...
static inline int is_leaf(struct bplus_node *node) { return node->type == BPLUS_TREE_LEAF; }
...
static void node_delete(struct bplus_tree *tree, struct bplus_node *node, struct bplus_node *left,
...
static inline void sub_node_update(struct bplus_tree *tree, struct bplus_node *parent, int index,
...
static void leaf_simple_insert(struct bplus_tree *tree, struct bplus_node *leaf, key_t key,
                               long data, int insert) {
...
static inline void non_leaf_simple_remove(struct bplus_tree *tree, struct bplus_node *node,
                                          int remove) {
...
static inline void leaf_simple_remove(struct bplus_tree *tree, struct bplus_node *leaf,
                                      int remove) {
...
long bplus_tree_get(struct bplus_tree *tree, key_t key) { return bplus_tree_search(tree, key); }
...
int bplus_open(char *filename) { return open(filename, O_CREAT | O_RDWR, 0644); }
...
static void draw(struct bplus_tree *tree, struct bplus_node *node, struct node_backlog *stack,
                 int level) {
    int i;
    for (i = 1; i < level; i++) {
        if (i == level - 1) {
