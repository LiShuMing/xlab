...
class MachineOp {
  public:
    /// return cpu cores(logical cores, not physical cores), return -1 if failed
    static int32_t num_of_available_cpu_cores();

    struct mem_info {
        int32_t total_kb;   // total_kb   total memory size
        int32_t free_kb;    // free_kb    free memory size
        int32_t buffers_kb; // buffers_kb buffers size
        int32_t cached_kb;  // cached_kb  cached size
        int32_t used_kb;    // used_kb    used size
                            // linux下等于total-free-buffers-cached而不是对应top命令中信息（top中的used是把buffers和cached都加里面了），@TODO
                            // macos下也是一样，比top中的小
  ...
  private:
    MachineOp();
    MachineOp(const MachineOp &);
    MachineOp &operator=(const MachineOp &);
};
...
inline int32_t MachineOp::num_of_available_cpu_cores() {
    // return ::get_nprocs();
    return sysconf(_SC_NPROCESSORS_CONF);
}

inline std::string MachineOp::host_name() {
    char buf[256] = {0};
...
class netdev {
  public:
    netdev() : in_bytes_(0), out_bytes_(0) {}

    uint64_t in_bytes_;
    uint64_t out_bytes_;

    bool parse(const std::string &interface) {
        std::string content = xlab::filepath_op::read_file("/proc/net/dev", 65535);
        if (content.empty()) {
            return false;
        }
...
class stat {
  public:
    ...
    bool parse() {
        std::string content = xlab::filepath_op::read_file("/proc/stat", 65535);
        if (content.empty()) {
            return false;
        }
...
static bool __parse_mem_info_attr_from_content(const std::string &content, const std::string &k,
                                               int *v) {
    ...
    if (v) {
        *v = std::atoi(content.c_str() + pos + k.length());
    }
...
inline bool MachineOp::obtain_mem_info(mem_info *mi) {
    if (!mi) {
        return false;
    }

    std::string content = xlab::filepath_op::read_file("/proc/meminfo", 65535);
    if (content.empty()) {
        return false;
    }

    if (!inner::__parse_mem_info_attr_from_content(content, "MemTotal:", &mi->total_kb)) {
        return false;
    }
    if (!inner::__parse_mem_info_attr_from_content(content, "MemFree:", &mi->free_kb)) {
        return false;
    }
    if (!inner::__parse_mem_info_attr_from_content(content, "Buffers:", &mi->buffers_kb)) {
        return false;
    }
    if (!inner::__parse_mem_info_attr_from_content(content, "Cached:", &mi->cached_kb)) {
        return false;
    }
...
inline int64_t MachineOp::boot_timestamp() {
    inner::stat st;
    if (!st.parse()) {
        return -1;
    }
...
inline int64_t MachineOp::up_duration_seconds() {
    inner::stat st;
    if (!st.parse()) {
        return -1;
    }
...
inline bool MachineOp::net_interface_bytes(const std::string &interface, uint64_t *in,
                                           uint64_t *out) {
    inner::netdev nd;
    if (!nd.parse(interface)) {
        return false;
    }
...
inline bool MachineOp::net_interfaces(std::set<std::string> *ifs) {
    if (!ifs) {
        return false;
    }
...
inline bool MachineOp::obtain_mem_info(mem_info *mi) {
    ...
    if (res != KERN_SUCCESS) {
        std::cerr << "host_page_size() failed.res:" << res << std::endl;
        return false;
    }
    ...
    if (res != KERN_SUCCESS) {
        std::cerr << "host_info() failed.res:" << res << std::endl;
        return false;
    }
    ...
    if (res != KERN_SUCCESS) {
        std::cerr << "host_statistics() failed.res:" << res << std::endl;
        return false;
    }

    mi->used_kb =
            (vm_info.active_count + vm_info.inactive_count + vm_info.wire_count) * page_size / 1024;
...
inline int64_t MachineOp::up_duration_seconds() { return std::time(NULL) - boot_timestamp(); }

inline bool MachineOp::net_interface_bytes(const std::string &interface, uint64_t *in,
                                           uint64_t *out) {
    if (interface.empty() || (!in && !out)) {
        return false;
    }
