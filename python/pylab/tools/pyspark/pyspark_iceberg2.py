...
target_iceberg_table = f"{catalog_name}.sql_test_db.lineitem_days_1000"
...
df.write \
    .format("iceberg") \
    .mode("append") \
    .save(f"{target_iceberg_table}")

spark.sql(f"""
SELECT COUNT(*) AS total_rows FROM {target_iceberg_table}
""").show()

spark.sql(f"""
SELECT l_returnflag, COUNT(*) AS count FROM {target_iceberg_table} 
