"""
Contains the :class:`base class <toydb.middlewares.Middleware>` for
middlewares and implementations.
"""
from typing import Optional

from toydb import Storage
...
class Middleware:
    ...
    Middlewares hook into the read/write process of toydb allowing you to
    ...
    def __call__(self, *args, **kwargs):
        ...
        Usually a user creates a new toydb instance like this::

            toydb(storage=StorageClass)

        The storage keyword argument is used by toydb this way::
        ...
        Using Middlewares, the user will call::
                                       ...
                                       v
            toydb(storage=Middleware(StorageClass))
                       ^
                       Already an instance!

        So, when running ``self.storage = storage(*args, **kwargs)`` Python
        now will call ``__call__`` and toydb will expect the return value to
        ...
        from toydb (``toydb(arg1, kwarg1=value, storage=...)``).
...
class CachingMiddleware(Middleware):
    """
    Add some caching to toydb.

    This Middleware aims to improve the performance of toydb by writing only
