...
import { getDefaultClaudeConfig } from "./Meet/ClaudeAPI";
...
export default class Views {
  ...
  public createOrUpdateModelsContainer() {
	...
	(publishSelectContainer as HTMLElement).style.color = '#222';
	  ...
	  if (curPublisher == "OpenAI") {
	    curAPIKey = Zotero.Prefs.get(`${config.addonRef}.openaiApiKey`) as string
	  } else if (curPublisher == "Claude-3") {
	    curAPIKey = Zotero.Prefs.get(`${config.addonRef}.claudeApiKey`) as string
	  } else if (curPublisher == "MiniMax") {
	    curAPIKey = Zotero.Prefs.get(`${config.addonRef}.minimaxApiKey`) as string
	  } else if (curPublisher == "Qwen") {
	    curAPIKey = Zotero.Prefs.get(`${config.addonRef}.qwenApiKey`) as string
	  } else if (curPublisher == "Gemini") {
	    curAPIKey = Zotero.Prefs.get(`${config.addonRef}.geminiApiKey`) as string
	  } else if (curPublisher == "DeepSeek") {
	    curAPIKey = Zotero.Prefs.get(`${config.addonRef}.deepseekApiKey`) as string
	  } else {
	    curAPIKey = curPublisherElement.apiKey
	  }
	  ...
	  if (curPublisher == "Local LLM") {
		 ...
		 const curPublisherElement = this.publisher2models.get(curPublisher)
			 ...
			 if (curPublisher == "OpenAI") {
			     ...
			     Zotero.Prefs.set(`${config.addonRef}.qwenApiKey`, (<HTMLInputElement>apiContainer).value)
			 } else if (curPublisher == "MiniMax") {
			     Zotero.Prefs.set(`${config.addonRef}.minimaxApiKey`, (<HTMLInputElement>apiContainer).value)
			 }
			 if (Zotero.isMac) {
    ...
    ztoolkit.UI.appendElement({
      ...
      listeners: [
	    ...
	    if (timer) {
              window.clearTimeout(timer)
              timer = undefined
              if (this.outputContainer) this.outputContainer.querySelector(".auxiliary")?.remove()
  ...
  private async execTag(tag: Tag) {
    Zotero.log("[PapersGPT] execTag called, tag: " + tag.tag)
    if (this.isInference) {
      Zotero.log("[PapersGPT] execTag skipped: isInference=true")
      return
    }
    ...
    Zotero.log("[PapersGPT] Full tag.text: " + tag.text.replace(/\n/g, "\\n"))
    let text = tag.text.replace(/^#.+\n/, "")
    Zotero.log("[PapersGPT] After removing header, text: " + text.replace(/\n/g, "\\n"))
    Zotero.log("[PapersGPT] Tag text length: " + text.length)

    // Check for ${ or ${ pattern
    const templateMatch = text.match(/\$\{[\s\S]+?\}/g)
    Zotero.log("[PapersGPT] Template matches (mode 1): " + (templateMatch?.length || 0))

    // Also try alternative pattern
    const templateMatch2 = text.match(/\$\\{[\s\S]+?\\}/g)
    Zotero.log("[PapersGPT] Template matches (mode 2 escaped): " + (templateMatch2?.length || 0))

    if (templateMatch) {
      Zotero.log("[PapersGPT] First match: " + templateMatch[0]?.substring(0, 100))
    }
    if (templateMatch2) {
      Zotero.log("[PapersGPT] First escaped match: " + templateMatch2[0]?.substring(0, 100))
    }
    ...
    // Handle ${...} pattern
    const matches1 = text.match(/\$\{[\s\S]+?\}/g) || []
    // Handle ${...} pattern (escaped backslash)
    const matches2 = text.match(/\$\\{[\s\S]+?\\}/g) || []
    const allMatches = [...matches1, ...matches2]

    Zotero.log("[PapersGPT] Total template matches: " + allMatches.length)

    for (let rawString of allMatches) {
      // Extract code inside ${...} or ${...}
      let codeString = rawString.replace(/^\$\{|\}$/g, "").replace(/^\$\\|\\}$/, "").replace(/\\}$/, "")
      Zotero.log("[PapersGPT] Template found: " + rawString.substring(0, 80))
      Zotero.log("[PapersGPT] Code to eval: " + codeString)
      try {
        const result = await window.eval(codeString)
        Zotero.log("[PapersGPT] Template result length: " + (result?.length || 0))
        text = text.replace(rawString, result || "")
      } catch (e) {
        Zotero.log("[PapersGPT] Template eval failed: " + e)
      }
    }
    Zotero.log("[PapersGPT] Final text before API call: " + text.substring(0, 200))
  ...
  private async callback() {
    ...
    this.isInNote = false
    const defaultOpenAIApiKey: string = Zotero.Prefs.get(`${config.addonRef}.openaiApiKey`) as string
    let openAIModelConfig: ModelConfig = {
    ...
    this.publisher2models.set("OpenAI", openAIModelConfig)
    this.publishers.push("OpenAI")

    const defaultClaudeApiKey: string = Zotero.Prefs.get(`${config.addonRef}.claudeApiKey`) as string
    // Use the new Claude configuration from Anthropic SDK module
    const claudeConfig = getDefaultClaudeConfig()
    let claudeModelConfig: ModelConfig = {
      models: claudeConfig.models,
      hasApiKey: claudeConfig.hasApiKey,
      apiKey: defaultClaudeApiKey,
      areModelsReady: new Map(),
      defaultModelIdx: 0,
      apiUrl: claudeConfig.apiUrl
    }

    this.publisher2models.set("Claude-3", claudeModelConfig)
    this.publishers.push("Claude-3")

    // Force update API URL if current publisher is Claude-3 (to apply new minimaxi URL)
    const currentPublisher = Zotero.Prefs.get(`${config.addonRef}.usingPublisher`) as string
    if (currentPublisher === "Claude-3") {
      Zotero.Prefs.set(`${config.addonRef}.usingAPIURL`, claudeModelConfig.apiUrl)
    }

    const defaultMiniMaxApiKey: string = Zotero.Prefs.get(`${config.addonRef}.minimaxApiKey`) as string
    let minimaxModelConfig: ModelConfig = {
      models: ["minimax-text-01", "minimax-text-01-v2", "minimax-m2.1", "minimax-embedding-01"],
      hasApiKey: true,
      apiKey: defaultMiniMaxApiKey,
      areModelsReady: new Map(),
      defaultModelIdx: 0,
      apiUrl: "https://api.minimax.chat/v1/text/chatcompletion_v2"
    }

    this.publisher2models.set("MiniMax", minimaxModelConfig)
    this.publishers.push("MiniMax")

    const defaultQwenApiKey: string = Zotero.Prefs.get(`${config.addonRef}.qwenApiKey`) as string
    let qwenModelConfig: ModelConfig = {
      models: ["qwen-turbo", "qwen-plus", "qwen-max", "qwen-max-longcontext"],
      hasApiKey: true,
      apiKey: defaultQwenApiKey,
      areModelsReady: new Map(),
      defaultModelIdx: 0,
      apiUrl: "https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions"
    }

    this.publisher2models.set("Qwen", qwenModelConfig)
    this.publishers.push("Qwen")
