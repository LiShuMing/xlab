...
## 2026-05-08 - mails.dev-inspired Uni-app frontend scaffold

### Follow-up: Local AI feedback preview

- Added `miniprogram/src/utils/aiFeedback.js` (40 lines): deterministic local feedback generator for record summaries, model-style responses, follow-up prompts, and chat fallback replies.
- Updated `miniprogram/src/pages/index/index.vue` (199 lines): saving a record now shows a `model feedback` terminal panel and creates a local preview record when backend auth/API is unavailable.
- Updated `miniprogram/src/pages/chat/index.vue` (163 lines): chat now adds a local assistant reply when the backend session or message call fails, so the product flow remains visible in H5 preview.
- Verification: `npm run build:mp-weixin` passed; `npm run build:h5` passed; manual H5 smoke test confirmed the feedback panel renders after saving a sample record.

### Follow-up: Local record detail fix

- Added `miniprogram/src/utils/localRecords.js` (24 lines): persists fallback records in local `uni` storage for H5/mini-program preview flows.
- Updated `miniprogram/src/api/request.js` (42 lines): adds a 3.5s request timeout so disconnected backend calls fail quickly.
- Updated `miniprogram/src/pages/index/index.vue` (200 lines): local fallback records are now saved and reloaded from local storage.
- Updated `miniprogram/src/pages/record/index.vue` (145 lines): `local-*` ids resolve from local storage, and missing pre-fix local records show a terminal-style missing-state instead of a spinner.
- Verification: `npm run build:mp-weixin` passed; `npm run build:h5` passed; manual H5 smoke test confirmed a newly saved local record opens in the detail page with content populated.

### Follow-up: Role selection and warmer therapist replies

- Added `miniprogram/src/utils/roles.js` (26 lines): local role state, built-in role metadata, and display-name helpers.
- Updated `miniprogram/src/pages/role/index.vue` (71 lines): role selection now persists locally and merges backend roles with built-in descriptions.
- Updated `miniprogram/src/components/RoleCard.vue` (70 lines): role cards now show descriptions so the choice is understandable.
- Updated `miniprogram/src/utils/aiFeedback.js` (69 lines): therapist responses are longer, warmer, and more emotionally validating; researcher remains concise and analytical.
- Updated `miniprogram/src/pages/chat/index.vue` (190 lines): chat reads the selected role on page show, resets sessions when role changes, and announces the new response perspective.
- Updated `miniprogram/src/pages/index/index.vue` (229 lines): record feedback uses the current role and provides a visible role switch entry point.
- Verification: `npm run build:mp-weixin` passed; `npm run build:h5` passed; manual H5 smoke test confirmed role selection persists and therapist fallback replies are richer.

### Follow-up: Chat error log analysis fixes

- Updated `miniprogram/src/components/ChatBubble.vue` (74 lines): assistant bubbles now render the role label stored on each message, preventing historical therapist replies from being relabeled as researcher after a role switch.
- Added `miniprogram/src/utils/localChat.js` (39 lines): persists local chat preview messages and creates welcome messages that match the selected role.
- Updated `miniprogram/src/pages/chat/index.vue` (203 lines): chat messages are saved locally, assistant messages keep `role_id` and `role_label`, and role-change announcements are persisted with their original role.
- Findings: current console logs only showed an old `Async component timed out after 60000ms` from the earlier local-record detail issue; after reload there were no new runtime errors, but the UI had role-label drift and no local chat persistence.
- Verification: `npm run build:mp-weixin` passed; `npm run build:h5` passed.

### Follow-up: Explicit role switching controls

- Updated `miniprogram/src/components/RoleCard.vue` (96 lines): role cards now include explicit `使用此角色` / `当前使用` buttons in addition to row tap selection.
- Updated `miniprogram/src/pages/role/index.vue` (98 lines): switching happens immediately in local state, shows instant feedback, and adds a `去对话` action for continuing with the selected role.
- Rationale: previous row-only tap behavior was too subtle and backend sync timeout made role switching feel unresponsive in local preview.
- Verification: `npm run build:mp-weixin` passed; `npm run build:h5` passed; manual H5 smoke test confirmed switching between researcher and therapist updates selection state immediately.

### Follow-up: 6-digit PIN login and account-scoped chat

- Added backend `POST /api/auth/pin-login`: `app/api/auth.py` (145 lines), `app/services/auth_service.py` (234 lines), and auth schemas (24/28 lines) now support six digit PIN login.
- Account identity is derived from `client_ip + pin` using SHA-256 and stored as the existing `User.openid`, so existing `user_id` chat/record isolation continues to apply.
- Added `miniprogram/src/pages/login/index.vue` (98 lines): a minimal login screen that accepts only six digits.
- Added `miniprogram/src/utils/authState.js` (50 lines): token, account id, account label, login guard, and account-scoped storage key helpers.
- Updated frontend auth API (51 lines) to call `/auth/pin-login` and fallback to local account mode when the backend is unavailable.
- Updated local chat/record storage (40/26 lines) to scope data by account id, preventing different six digit accounts from sharing preview conversations.
- Updated `index.vue` (240 lines), `chat/index.vue` (214 lines), and `role/index.vue` (102 lines) to require login and use account-scoped data.
- Verification: backend auth files passed `python -m py_compile`; `npm run build:mp-weixin` passed; `npm run build:h5` passed; manual H5 smoke test confirmed PIN login and that `123456` chat messages do not appear under `654321`.

### Follow-up: Ctrl+Enter chat sending

- Updated `miniprogram/src/pages/chat/index.vue` (235 lines): H5 preview now listens for page-level `Ctrl+Enter` / `Cmd+Enter` while the chat textarea is focused and sends the message immediately.
- Kept normal Enter behavior unchanged for multi-line composition.
- Verification: `npm run build:mp-weixin` passed; `npm run build:h5` passed; manual H5 smoke test confirmed `Ctrl+Enter` submits and clears the composer.

### Follow-up: Local backend linkage mode

- Added `py-ego-miniapp/app/utils/memory_redis.py`: a tiny async Redis-compatible cache used when `REDIS_URL=memory://`, so PIN auth can run locally without a Redis daemon.
- Updated backend startup in `py-ego-miniapp/app/main.py`: when using a SQLite `DATABASE_URL`, the API creates local tables automatically.
- Updated `py-ego-miniapp/app/services/memory_service.py`: pgvector memory search now gracefully no-ops outside PostgreSQL, keeping local SQLite chat and record flows usable.
- Updated Python 3.9 compatibility for backend annotations in model/schema files and added `greenlet` to backend requirements for SQLAlchemy async.
- Updated `py-ego-miniapp/app/services/chat_service.py`: if no LLM key/provider is available, backend chat now returns role-aware warm fallback replies instead of a cold temporary failure message.
- Verification: backend `compileall` passed; local API started on `http://127.0.0.1:8000`; `GET /health`, `POST /api/auth/pin-login`, chat session creation, and message sending all passed with SQLite + memory Redis; `npm run build:h5` passed.

### Summary

Created the `miniprogram/` Uni-app frontend described by `docs/superpowers/specs/2026-03-24-wechat-mini-program-design.md` and adapted it to the mails.dev visual language. The new UI uses a warm paper background, black hairline grid, mono status text, large serif page titles, square controls, terminal-style panels, and a single green status accent.

### Files Created / Modified

- `miniprogram/.gitignore` (6 lines): ignores local build artifacts while unignoring JSON files needed by Uni-app.
- `miniprogram/README.md` (25 lines): documents style direction, development command, and API expectation.
- `miniprogram/index.html` (12 lines): adds the Vite entry needed for H5 visual preview.
- `miniprogram/package.json` (22 lines): adds Uni-app/Vue/Vite dependencies and WeChat/H5 build scripts.
- `miniprogram/package-lock.json` (7747 lines): locks the verified frontend dependency graph.
- `miniprogram/vite.config.js` (6 lines): configures the Uni-app Vite plugin.
- `miniprogram/src/App.vue` (17 lines), `main.js` (9 lines), `manifest.json` (15 lines), `pages.json` (76 lines): app entry, WeChat manifest, and routes/tab navigation.
- `miniprogram/src/static/styles/theme.css` (218 lines): shared mails.dev-inspired design tokens, layout primitives, and H5/mobile overflow normalization.
- `miniprogram/src/api/*.js` (152 lines total): auth, records, chat, roles, request wrapper, and mini-program-safe query builder.
- `miniprogram/src/components/*.vue` (239 lines total): navigation/status bar, record card, chat bubble, voice input, and role card.
- `miniprogram/src/pages/**/*.vue` (633 lines total): record home, chat, timeline, profile, role selection, and record detail/edit pages.
- `miniprogram/src/utils/date.js` (20 lines), `query.js` (7 lines): date labels and query serialization helpers.

### Architecture Decisions

- Added the frontend as a separate `miniprogram/` app so the existing FastAPI backend remains untouched.
- Kept the UI API-facing but resilient for local development: pages render fallback data or empty states if backend auth/data is unavailable.
- Used the API paths already implemented under `py-ego-miniapp/app/api`: `/auth`, `/records`, `/chat`, and `/roles`.
- Avoided image-heavy decoration and rounded card stacks. The interface is structured as functional grid panels, closer to mails.dev's engineering/product aesthetic than a soft lifestyle app.
- Added `.gitignore` exceptions because the repository root ignores `*.json`, but Uni-app requires `package.json`, `manifest.json`, and `pages.json` to be versioned.

### Breaking Changes

None. Existing backend, CLI, and data files were not modified.

### Test Results

- `npm install --package-lock-only`: passed after pinning available DCloud alpha package versions.
- `npm install`: passed with deprecation warnings from transitive `phin` packages.
- `npm run build:mp-weixin`: passed. Build output is available locally under `miniprogram/dist/build/mp-weixin` and ignored by git.
- `npm run build:h5`: passed. H5 preview support is available for browser-based visual review.
