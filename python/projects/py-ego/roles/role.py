"""Pydantic model for rich agent role definition."""
...
class RoleStyle(BaseModel):
    """UI style hints for a role.

    UI 样式提示，定义角色在终端中的视觉风格。
    """

    primary_color: str = "CYAN"  # Main theme color for the role / 角色主色调
    secondary_color: str = "BRIGHT_CYAN"  # Accent color for highlights / 高亮强调色
    icon_color: str = "BRIGHT_GREEN"  # Color for role icon / 角色图标颜色


class RoleKnowledge(BaseModel):
    """Role-specific knowledge base for richer responses.
    ...
    domain_expertise: list[str] = Field(
        ...
        description="Professional domains the role specializes in, e.g., ['CBT', 'Mindfulness'] / 专业领域",
    )
    key_concepts: list[str] = Field(
        default_factory=list,
        description="Core concepts and principles the role is familiar with / 核心概念",
    )
    classic_quotes: list[str] = Field(
        default_factory=list,
        description="Memorable quotes the role can cite in conversations / 经典语录",
    )
    references: list[str] = Field(
        default_factory=list,
        description="Recommended books, papers, or resources / 推荐参考资源",
    )
...
class RolePersonality(BaseModel):
    ...
    background: str = Field(
        default="",
        description="Professional background and life story that shapes the role's perspective / 背景故事",
    )
    traits: list[str] = Field(
        default_factory=list,
        description="Personality traits, e.g., ['warm', 'patient', 'professional'] / 性格特点",
    )
    speaking_style: str = Field(
        default="",
        description="How the role communicates, e.g., 'warm, empathetic, non-judgmental' / 说话风格",
    )
    catchphrases: list[str] = Field(
        default_factory=list,
        description="Signature phrases the role frequently uses / 口头禅",
    )


class RoleExample(BaseModel):
    """Few-shot example dialogue for style imitation.

    示例对话，用于让 LLM 模仿角色的说话风格。
    """

    user_input: str = Field(..., description="Example user message / 用户输入示例")
    assistant_response: str = Field(..., description="Example assistant reply / 助手回复示例")
    context: str | None = Field(
        default=None,
        description="Situation context explaining the dialogue scenario / 情境说明",
    )


class Role(BaseModel):
    """Definition of a rich agent role/persona.

    角色定义，包含完整的人设信息：
    - System prompt for LLM behavior / 系统提示词
    - Personality (background, traits, speaking style) / 性格风格
    - Knowledge base (expertise, quotes, references) / 知识库
    - Example dialogues (few-shot learning) / 示例对话
    - Independent memory and layered profile / 独立记忆和分层画像
    - UI styling
    """

    id: str = Field(..., description="Unique identifier, e.g., 'therapist', 'researcher'")
    ...
    profile_prompt: str = Field(default="", description="Profile update prompt")

    # NEW: Rich character dimensions
    personality: RolePersonality = Field(
        default_factory=RolePersonality,
        description="角色性格和说话风格",
    )
    knowledge: RoleKnowledge = Field(
        default_factory=RoleKnowledge,
        description="角色知识库",
    )
    examples: list[RoleExample] = Field(
        default_factory=list,
        description="示例对话，用于风格模仿",
    )

    style: RoleStyle = Field(default_factory=RoleStyle, description="UI styling")

    @property
    def memory_prefix(self) -> str:
        """Prefix for memory files."""
        return f"{self.id}_"

    @property
    def profile_filename(self) -> str:
        """Filename for role-specific profile."""
        return f"profile_{self.id}.md"

    @property
    def relationship_filename(self) -> str:
        """Filename for relationship memory."""
        return f"relationship_{self.id}.md"

    def build_full_system_prompt(self, relationship_context: str = "") -> str:
        """Build the complete system prompt with all dimensions.

        Args:
            relationship_context: The relationship memory context.
    ...
    def from_yaml(cls, path: Path) -> Role:
        """Load a role from YAML file."""
        with open(path, "r", encoding="utf-8") as f:
            data = yaml.safe_load(f)

        # Parse nested objects
        if "style" in data:
            data["style"] = RoleStyle(**data["style"])
        if "personality" in data:
            data["personality"] = RolePersonality(**data["personality"])
        if "knowledge" in data:
            data["knowledge"] = RoleKnowledge(**data["knowledge"])
        if "examples" in data:
            data["examples"] = [RoleExample(**ex) for ex in data["examples"]]

        return cls(**data)

    @classmethod
    def load_all(cls, roles_dir: Path) -> dict[str, Role]:
        """Load all roles from a directory."""
        roles: dict[str, Role] = {}

        # Load from combined roles.yaml
        combined_file = roles_dir / "roles.yaml"
        if combined_file.exists():
            with open(combined_file, "r", encoding="utf-8") as f:
                data = yaml.safe_load(f)
            for role_data in data.get("roles", []):
                role = cls._parse_role_data(role_data)
                roles[role.id] = role

        # Load individual role files
        for yaml_file in roles_dir.glob("*.yaml"):
            if yaml_file.name == "roles.yaml":
                continue
            role = cls.from_yaml(yaml_file)
            roles[role.id] = role

        return roles

    @classmethod
    def _parse_role_data(cls, data: dict[str, Any]) -> Role:
        """Parse role data from dict."""
        if "style" in data:
            data["style"] = RoleStyle(**data["style"])
        if "personality" in data:
            data["personality"] = RolePersonality(**data["personality"])
...
# ============================================================================
# Predefined Roles with Rich Details
# ============================================================================
...
ROLE_THERAPIST = Role(
    ...
    profile_prompt="记录用户的情绪模式、压力来源、人际关系状况、自我认知特点、心理健康目标。",
    style=RoleStyle(primary_color="CYAN", secondary_color="BRIGHT_CYAN", icon_color="BRIGHT_GREEN"),
)
...
ROLE_RESEARCHER = Role(
    ...
    profile_prompt="记录用户的研究兴趣、专业领域、常用研究方法论偏好、报告格式偏好。",
    style=RoleStyle(primary_color="BLUE", secondary_color="BRIGHT_BLUE", icon_color="BRIGHT_GREEN"),
)
...
ROLE_LEARNER = Role(
    ...
    profile_prompt="记录用户的学习目标、当前技能水平、学习风格偏好、可用学习时间、偏好的学习资源类型。",
    style=RoleStyle(primary_color="GREEN", secondary_color="BRIGHT_GREEN", icon_color="BRIGHT_YELLOW"),
)
...
ROLE_PHILOSOPHER = Role(
    ...
    profile_prompt="记录用户的人生困惑、价值观念、哲学兴趣方向、正在思考的核心问题。",
    style=RoleStyle(primary_color="MAGENTA", secondary_color="BRIGHT_MAGENTA", icon_color="BRIGHT_CYAN"),
)
