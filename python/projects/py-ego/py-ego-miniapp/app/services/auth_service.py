...
class AuthService:
    ...
    async def login_with_pin(self, pin: str, client_ip: str) -> tuple[User, str, str]:
        """Login using a 6-digit PIN scoped by client IP.

        Args:
            pin: Six digit login code provided by the user.
            client_ip: Client IP address resolved at the API boundary.

        Returns:
            tuple[User, str, str]: User, access token, refresh token.
        """
        account_key = self._pin_account_key(pin, client_ip)
        user = await self._get_or_create_user(account_key)
        user.nickname = f"PIN-{pin}"
        user.last_active_at = datetime.now(timezone.utc).replace(tzinfo=None)
        await self._db.commit()
        await self._db.refresh(user)

        access_token = create_access_token({"sub": str(user.id), "login": "pin"})
        refresh_token = create_refresh_token({"sub": str(user.id), "login": "pin"})
    ...
    def _pin_account_key(self, pin: str, client_ip: str) -> str:
        normalized_ip = client_ip.strip() or "unknown"
        digest = hashlib.sha256(f"{normalized_ip}:{pin}".encode("utf-8")).hexdigest()
        return f"pin:{digest[:48]}"
