...
class ChatService:
    ...
    async def send_message(
        ...
        try:
            ...
            reply = self._build_fallback_reply(data.content, session.role_id)
    ...
    def _build_fallback_reply(self, content: str, role_id: str | None) -> str:
        """Build a local reply when the configured LLM is unavailable."""
        normalized = content.strip() or "这件事"

        if role_id == "researcher":
            return (
                f"我先按研究员的方式帮你拆一下：你提到「{normalized}」。"
                "这里至少有三个可观察层面：事实发生了什么、你当时的感受是什么、"
                "以及下一步可以验证或调整什么。建议先写下一个最小行动，比如补充一个证据、"
                "做一次小实验，或者把这个问题拆成今天能完成的一步。"
            )

        if role_id == "learner":
            return (
                f"我听到你正在面对「{normalized}」。我们可以把它当成一个学习任务："
                "先确认目标，再找出现在最卡的一个点，最后给自己一个 15 分钟就能完成的小练习。"
                "不用一下子做到完美，先让系统开始运转起来。"
            )

        if role_id == "philosopher":
            return (
                f"你说到「{normalized}」，这背后也许不只是一个事件，而是在提醒你："
                "此刻什么对你重要，什么正在消耗你。可以慢一点问自己，"
                "如果不急着给答案，我真正想守住的价值是什么？"
            )

        return (
            f"我在这里，先陪你把这件事放稳一点。你提到「{normalized}」，"
            "听起来它确实占用了你不少心力。先不用急着解决全部，我们可以从两个很小的问题开始："
            "此刻最明显的感受是什么？身体哪里最紧？如果今天只做一件能让你轻一点的事，"
            "那件事可以小到什么程度？你已经把它说出来了，这本身就是在照顾自己。"
        )
