...
async def _run_sync_summarize(target_date: str) -> dict[str, Any]:
    """
    Run sync and summarize for a specific date.

    This is a background task that:
    1. Runs incremental sync to fetch new emails
    2. Runs summarize for the target date
    3. Builds the digest
    ...
    from my_email.db.repository import get_sync_state, save_summary, save_thread_summary
...
async def _build_digest_for_date(target_date: str) -> bool:
    """
    Build digest for a date after summarization.
    ...
    from my_email.db.repository import save_digest
...
def _trigger_full_pipeline(target_date: str) -> None:
    """
    Trigger sync + summarize + digest pipeline for a date.

    Updates task_status with progress.
    """
    async def _run():
        _task_status[target_date] = {"status": "running", "step": "sync"}

        # Run sync and summarize
        result = await _run_sync_summarize(target_date)
        ...
        # Build digest
        _task_status[target_date]["step"] = "digest"
        ok = await _build_digest_for_date(target_date)
...
async def generate_digest(target_date: str, background_tasks: BackgroundTasks):
    ...
    background_tasks.add_task(_trigger_full_pipeline, target_date)
