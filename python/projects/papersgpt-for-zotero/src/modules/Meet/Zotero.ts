...
export async function getRelatedText(queryText: string) {
  Zotero.log("[PapersGPT] getRelatedText called with query: " + queryText?.substring(0, 50))
  const usingPublisher = Zotero.Prefs.get(`${config.addonRef}.usingPublisher`)
  Zotero.log("[PapersGPT] usingPublisher: " + usingPublisher)
  if (usingPublisher != "Local LLM") {
    ...
    switch (Zotero_Tabs.selectedIndex) {
      case 0:
        ...
        key = md5(ZoteroPane.getSelectedItems().map(i => i.key).join(""))
        Zotero.log("[PapersGPT] Processing selected items, key: " + key)
      ...
      default:
        ...
        key = pdfItem.key
        Zotero.log("[PapersGPT] Processing PDF, key: " + key)
        docs = cache[key] || await pdf2documents(key)
        break
    }
    cache[key] = docs
    Zotero.log("[PapersGPT] Docs count: " + docs?.length)
    docs = await similaritySearch(queryText, docs, { key }) as Document[]
    Zotero.log("[PapersGPT] After similarity search, docs count: " + docs?.length)
