...
function parseJsonResults(publisher2models: Map<string, ModelConfig>, publishers:string[], supportedLLMsJson: []) {
  ...
  for (var i = 0; i < supportedLLMsJson.length; i++) {
	...
	} else if (publisher == "MiniMax") {
	    apiKey = Zotero.Prefs.get(`${config.addonRef}.minimaxApiKey`) as string
	} else if (publisher == "Qwen") {
	    apiKey = Zotero.Prefs.get(`${config.addonRef}.qwenApiKey`) as string
