...
_TEMPLATES = ["summary", "deep-dive", "blog", "interview-note"]
...
def register(app: typer.Typer) -> None:
    ...
    def report(
        ...
        if not paths.paper_schema.exists():
            console.print(f"[red]Error:[/red] No paper schema found in {run_dir}. Run analyze first.")
            raise typer.Exit(1)
        ...
        llm = LLMClient(model=resolved_model, max_tokens=3000, temperature=0.1, timeout=timeout)

        console.print(f"[blue]Generating {template} report for:[/blue] {schema.title[:60]!r}")
