...
class ResearchAgent:
    ...
    async def research(
        ...
        # 2. Run intelligence tools based on mode
        tasks: dict[str, Any] = {}

        # Fact verification (Phase 2)
        if (
        ...
        # TODO: Phase 3 - Add competition, history, docs tools
        # if options.enable_competition and self.tools.competition is not None:
        #     tasks["competition"] = self.tools.competition.run(product_name, options.competitors)

        # if options.enable_history and self.tools.history is not None:
        #     tasks["history"] = self.tools.history.run(product_name)

        # if options.enable_docs_analysis and self.tools.docs_analyzer is not None:
        #     tasks["docs"] = self.tools.docs_analyzer.run(product_name)

        # Execute tools in parallel if any
        if tasks:
    ...
    def create_default(
        ...
        if enable_fact_verify and api_key:
