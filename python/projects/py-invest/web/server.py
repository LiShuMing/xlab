...
class DashboardHandler(SimpleHTTPRequestHandler):
    ...
    def _handle_analyze_stock(self):
        """Generate one investment report for an input stock."""
        try:
            body = self._read_json_body()
            stock_code = (body.get('stock') or body.get('stock_code') or '').strip()
            query = (body.get('query') or '价值投资分析').strip()
            lang = (body.get('lang') or 'zh').strip()
            mode = (body.get('mode') or 'fast').strip()
            use_cache = bool(body.get('use_cache', True))

            if not stock_code:
                self._send_json({"success": False, "error": "stock is required"}, status=400)
                return

            if use_cache:
                cached = self._get_cached_report(stock_code, mode)
                if cached:
                    self._send_json({"success": True, "cached": True, **cached})
                    return

            report = asyncio.run(self._async_analyze_stock(stock_code, query, lang, mode))
            self._send_json({"success": True, **report})
        except Exception as e:
            self._send_json({"success": False, "error": str(e)}, status=500)

    def _get_cached_report(self, stock_code: str, mode: str) -> dict | None:
        """Return today's cached report when available."""
        try:
    ...
    async def _async_analyze_stock(self, stock_code: str, query: str, lang: str, mode: str) -> dict:
        """Run the existing py-invest agent pipeline for one stock."""
...
def main():
    ...
    # Keep the batch auto-analyzer opt-in. The Liminalis client triggers
    # single-stock analysis on demand, so server startup should be quiet.
    auto_analyzer = None
    if os.getenv("PY_INVEST_AUTO_ANALYZER") == "1":
