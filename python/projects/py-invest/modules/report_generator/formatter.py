...
<body>
    ...
    def _json_safe(value):
        """Convert report values into JSON-serializable objects."""
        if is_dataclass(value):
            return ReportFormatter._json_safe(asdict(value))
        if isinstance(value, dict):
            return {str(k): ReportFormatter._json_safe(v) for k, v in value.items()}
        if isinstance(value, (list, tuple)):
            return [ReportFormatter._json_safe(v) for v in value]
    ...
    def _to_json(report: Report, lang: str = "zh") -> str:
        ...
        data = {
            ...
            "raw_data": ReportFormatter._json_safe(report.raw_data),
            "sections": [
