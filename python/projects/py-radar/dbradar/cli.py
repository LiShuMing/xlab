...
def _data_dir_from_config(config: Config) -> Path:
...
@click.option(
    ...
    "-p",
    default="",
    help="Optional product/project/source hint, e.g. PostgreSQL or Daniel Lemire",
)
@click.option(
    "--source",
    "-s",
    default="",
    help="Optional source name hint",
)
@click.option(
    "--tag",
    "tags",
    multiple=True,
    help="Optional tag hint. Can be passed multiple times.",
)
@click.option(
    "--tags",
    "tags_csv",
    default="",
    help="Comma-separated tag hints, e.g. optimizer,storage",
)
@click.option(
    "--note",
    "-n",
    default="",
    help="Optional note for the LLM analyzer",
)
@click.option(
    "--json",
    "json_output",
...
@click.option(
    "--server-url",
    envvar="RADAR_SERVER_URL",
    default="",
    help="Submit through a running py-radar server, e.g. http://localhost:5000",
)
@click.option(
    "--admin-user",
    envvar="RADAR_ADMIN_USER",
    default="admin",
    help="Admin username for --server-url mode",
)
@click.option(
    "--admin-password",
    envvar="RADAR_ADMIN_PASSWORD",
    default="",
    help="Admin password for --server-url mode",
)
@click.option(
    "--wait/--no-wait",
    default=True,
    help="Wait for server-side job completion in --server-url mode",
)
@click.option(
    "--timeout",
    default=180,
    type=int,
    help="Seconds to wait for server-side job completion",
)
@click.pass_obj
def add_url(
    config: Config,
    ...
    """Add one URL, analyze it with the configured LLM, and save it to DuckDB."""
    tag_values = list(tags)
    if tags_csv:
        tag_values.extend(tag.strip() for tag in tags_csv.split(",") if tag.strip())

    if not config.api_key or not config.base_url:
        click.echo(
            "Warning: no LLM config found; falling back to extractor summary. "
            "Set LLM_API_KEY/LLM_BASE_URL in ~/.env to enable LLM analysis.",
            err=True,
        )
    ...
    if server_url:
        ...
        if json_output:
            click.echo(json.dumps(result, ensure_ascii=False, default=_json_default))
    ...
    if not json_output:
        click.echo(f"Adding URL: {url}")
        click.echo(f"Data directory: {data_dir}")
        click.echo(f"Job: {job_id}")
        click.echo("Fetching, extracting, analyzing, and saving...")

    result = ingest_link(job_id, request, data_dir, submitted_by="cli")

    if json_output:
        click.echo(json.dumps({"job_id": job_id, **result}, ensure_ascii=False, default=_json_default))
    ...
    if status == "completed":
        ...
        click.echo(f"Saved item: {item.get('id')}")
        click.echo(f"Title: {item.get('title')}")
        click.echo(f"Product: {item.get('product')}")
        click.echo(f"Type: {item.get('content_type')}")
        if item.get("tags"):
            click.echo(f"Tags: {', '.join(item.get('tags', []))}")
        if item.get("summary"):
            click.echo("")
            click.echo(item.get("summary"))
    elif status == "duplicate":
        item = result.get("item", {})
        click.echo(f"URL already exists: {item.get('id')}")
        click.echo(f"Title: {item.get('title')}")
    else:
...
def _add_url_via_server(
    ...
    with httpx.Client(base_url=server_url, timeout=30, follow_redirects=True) as client:
        login = client.post(
            "/api/admin/login",
            json={"username": admin_user, "password": admin_password},
        )
        if login.status_code >= 400:
            raise click.ClickException(f"admin login failed: {login.text}")
        ...
        if response.status_code >= 400:
            raise click.ClickException(f"submit failed: {response.text}")

        submitted = response.json()
        job = submitted.get("job", {})
        job_id = submitted.get("jobId") or job.get("id")

        if not json_output:
            click.echo(f"Submitted to server: {server_url}")
            click.echo(f"Job: {job_id}")

        if not wait or not job_id:
            return {"job_id": job_id, "status": submitted.get("status"), "job": job}

        deadline = time.time() + timeout
        while time.time() < deadline:
            job_response = client.get(f"/api/admin/radar/jobs/{job_id}")
            if job_response.status_code >= 400:
                raise click.ClickException(f"job poll failed: {job_response.text}")
            ...
            if status in {"completed", "duplicate", "failed"}:
                result = {"job_id": job_id, "status": status, "job": job}
                if status == "failed":
                    result["error"] = job.get("error")
                return result
            time.sleep(2)

        return {"job_id": job_id, "status": "timeout", "job": job}
