...
- `--language` / `-l` - Output language: `en` (English) or `zh` (中文)
...
GOOGLE_API_KEY="your-google-api-key"  # Google Custom Search API
GOOGLE_CX="your-search-engine-id"     # Google Custom Search Engine ID
NEWSAPI_KEY="your-newsapi-key"        # NewsAPI.org API key
```

### Enhanced Data Sources

When `GOOGLE_API_KEY` and `GOOGLE_CX` are configured, the system will automatically search Google for recent news about configured products.

When `NEWSAPI_KEY` is configured, the system will fetch relevant news articles from NewsAPI.

These enhanced sources provide additional high-value content beyond the configured URLs.
