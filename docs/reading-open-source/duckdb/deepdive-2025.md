...
**Compressed Materialization**（v0.9 引入）：在 ORDER BY、GROUP BY / Hash Aggregate 等需要保存中间结果的算子中，DuckDB 会在数据进入这些算子前，对部分整数和短字符串做低成本压缩（如按实际范围缩窄整型宽度、将短字符串编码为更紧凑的固定宽度表示）。这样可以显著减小中间结果和哈希表/排序缓冲区的体积，改善 cache locality，并减少因内存不足而触发 spill-to-disk 的概率，从而提升大查询的稳定性与性能。 ￼
