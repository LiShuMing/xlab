...
- Trino 的产品重心已经从“更快的 Hive SQL 前端”转向“开放 Lakehouse 与企业联邦查询入口”。
- Fault-Tolerant Execution 改变了它的工作负载边界，使一部分批处理和 ETL 能进入 Trino，而不再完全依赖 Spark。
- Iceberg、Delta、REST Catalog、Unity/Snowflake/S3 Tables 等适配能力说明，Trino 的护城河越来越依赖开放表格式和 Connector 生态深度。
- Starburst 的商业化价值主要在平台层：RBAC、Insights、Warp Speed、Data Products、Galaxy 和多云托管，而不是重写 Trino 执行器内核。
