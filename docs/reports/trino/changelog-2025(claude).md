...
- Trino 近三年的关键变化是从交互式 SQL-on-Hadoop 引擎扩展为开放 Lakehouse 查询平面，核心支点是 FTE、Iceberg/Delta 深度支持和 Connector pushdown。
- 它没有选择 Velox/Prestissimo 式 native executor 路线，而是继续强化 JVM Page/Block 模型、Connector SPI、对象存储读路径和运行时容错。
- Trino 的优势是 data gravity friendly：数据不必先搬入专用存储，就能通过统一 SQL 查询多种湖仓、事务库和云数仓。
- 天花板同样清晰：没有自有存储层意味着性能高度依赖远端 I/O、统计质量、pushdown 深度和跨源网络成本；它不会在纯本地 OLAP 上击败 ClickHouse/StarRocks。
