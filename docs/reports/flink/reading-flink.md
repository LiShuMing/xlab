...
- Flink 的核心价值不在单个算子性能，而在 `StreamOperator` 生命周期、checkpoint barrier、状态后端和 SQL planner 共同定义的长期运行语义。
- Operator 生命周期决定了状态初始化、快照、恢复、输入结束和资源释放的边界，是理解 Flink exactly-once、反压和有界/无界统一执行的入口。
- SQL 增量算子的本质是把 relational algebra 映射到带状态的 streaming operator DAG；Join、Aggregate、Top-N、Window 等算子的复杂度主要来自状态规模和更新语义。
- 对数据库系统的启示是：实时物化视图或流式 ETL 不能只看 SQL 语法覆盖度，必须同时评估 checkpoint 成本、状态访问模式、watermark 语义和恢复路径。
