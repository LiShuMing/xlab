...
- ClickHouse 的算子性能优势来自大量面向数据类型、基数、内存布局和 CPU cache 行为的特化路径，而不是单一“向量化执行”标签。
- Aggregate 和 Join 是最能体现 ClickHouse 工程哲学的部分：它通过多种 HashTable、two-level aggregation、Arena、低基数字典和外部溢写组合，在高吞吐与可控内存之间做动态取舍。
- Sort 与 Window 的性能边界更依赖 pipeline breaker、外部排序、内存阈值和分区规模；这些路径决定了 ClickHouse 在 BI Top-N、报表窗口函数和大宽表排序场景中的稳定性。
- 对 StarRocks 的启示是：算子优化必须同时覆盖布局、调度、spill、编码和语义改写；仅做局部 SIMD 或哈希表替换很难复制 ClickHouse 的整体收益。
