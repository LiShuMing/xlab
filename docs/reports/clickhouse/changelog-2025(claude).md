...
- ClickHouse 2025 的主线不是单点性能优化，而是从高性能 OLAP 引擎扩展为实时 Lakehouse 查询、搜索、向量检索与可观测性平台。
- Iceberg、Delta Lake、Unity/Glue/REST Catalog 的增强，说明 ClickHouse 正在争夺开放表格式上的一等计算引擎位置。
- Analyzer、JOIN reorder、lazy materialization、skip index 等优化，把 ClickHouse 从“写对 SQL 才快”推向更成熟的数据库优化器路线。
- 商业上，ClickHouse Cloud 与 ClickStack 把开源内核能力包装成更明确的云数仓和可观测性产品入口。
