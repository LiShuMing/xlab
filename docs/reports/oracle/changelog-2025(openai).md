...
- Oracle 最近几年的主线是把传统企业数据库能力包装为自治数据平台：调优、扩缩容、备份、恢复、补丁、容灾和访问控制逐步服务化。
- 23ai/26ai 把 AI Vector Search、JSON Relational Duality、Select AI 和 agent workflow 推到产品中心，说明 Oracle 正在把数据库定位为企业 AI 数据运行时。
- Iceberg REST Catalog、Data Lake Accelerator 和外部表能力体现了“有限开放”策略：接受开放湖仓生态，但尽量把治理、查询和 AI 入口拉回 Oracle 控制面。
- 商业定位仍然围绕高价值企业负载：关键系统可靠性、合规、安全、灾备、Exadata 性能与 DBA 经验产品化，而不是单纯追逐开源开发者心智。
