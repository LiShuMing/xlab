...
- Velox 的价值不在于提供完整数据库，而在于抽象出可复用的 C++ 向量化执行层，供 Presto、Spark Gluten、内部数据平台复用同一套算子内核。
- 与 StarRocks 这类完整 MPP 数据库相比，Velox 的边界更低层：它解决 Vector、Expression、HashJoin、HashAgg、MemoryPool、Spill 和 Connector 读写，不负责 SQL、Catalog、分布式优化器和事务语义。
- StarRocks 可以借鉴 Velox 的统一向量抽象、内存仲裁、spill 接口和跨引擎复用策略，但不能简单替换自身执行层；数据库级优化器、存储格式和调度语义仍需要本系统闭环。
- 真正值得关注的是 Velox 生态：Prestissimo 与 Gluten 表明未来执行引擎可能从“每个系统自研”转向“共享 native kernel + 上层系统差异化”。
