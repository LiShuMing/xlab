...
- TiDB 7.x 到 8.x 的核心变化，是从“HTAP 能力拼图”走向生产稳定性：TiFlash Pipeline、Resource Control、TTL、CDC、PITR 和可观测性持续补强。
- TiFlash 的向量化与 MPP 演进，使 TiDB 在事务库旁路分析上的能力更完整，但其 OLAP 性能上限仍受 TiKV/TiFlash 双存储、数据同步和调度边界制约。
- 存储层重点从功能扩张转向 MVCC amplification、RocksDB compaction、Raft snapshot、GC 与热点写入治理，这反映出 TiDB 已进入大规模生产负载的深水区。
- Vector Search 与 AI-ready 叙事是新增长点，但短期更像 MySQL 兼容 HTAP 平台的能力补齐，而不是对专用向量数据库或 OLAP 引擎的替代。
